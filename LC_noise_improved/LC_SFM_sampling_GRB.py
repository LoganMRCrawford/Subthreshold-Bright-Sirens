...
parser.add_argument('--psd-source', choices=('bayesline', 'welch'), default='bayesline',
                help='PSD supplied to the likelihood (default: bayesline).')
parser.add_argument('--bayesline-psd-dir',
                default='/home/dp429/dp429/dc-craw2/assist_bayeswave',
                help='Directory containing glitch_median_PSD_<IFO>.dat files.')
parser.add_argument('--bayesline-segment-duration', type=float, default=128.0,
                help='BayesWave likelihood segment duration in seconds (default: 128).')
parser.add_argument('--output-dir', default=None,
                    help='Directory for sampler outputs and PSD comparison plots.')
...
output_dir = args.output_dir or os.path.join(
    os.path.dirname(os.path.abspath(__file__)), 'results', grb_name
)
...
def load_bayesline_psd(ifo_name, frequencies):
    """Load a BayesWavePost median PSD and interpolate it onto likelihood bins."""
    filename = os.path.join(args.bayesline_psd_dir, f'glitch_median_PSD_{ifo_name}.dat')
    psd_data = np.loadtxt(filename, usecols=(0, 1))
    bayesline_frequencies = psd_data[:, 0]
    bayesline_psd = psd_data[:, 1] * 2.0 / args.bayesline_segment_duration

    valid = (
        np.isfinite(bayesline_frequencies)
        & np.isfinite(bayesline_psd)
        & (bayesline_psd > 0)
    )
    bayesline_frequencies = bayesline_frequencies[valid]
    bayesline_psd = bayesline_psd[valid]
    unique_frequencies, inverse, counts = np.unique(
        bayesline_frequencies, return_inverse=True, return_counts=True
    )
    bayesline_psd = np.bincount(
        inverse, weights=bayesline_psd
    ) / counts
    bayesline_frequencies = unique_frequencies
    if bayesline_frequencies.size < 2:
        raise ValueError(f'{filename} contains fewer than two valid PSD bins')
    frequency_tolerance = 2.0 * np.median(np.diff(bayesline_frequencies))
    active_frequencies = frequencies[(frequencies >= fmin) & (frequencies <= fmax)]
    if (
        active_frequencies[0] < bayesline_frequencies[0]
        or active_frequencies[-1] > bayesline_frequencies[-1] + frequency_tolerance
    ):
        raise ValueError(
            f'{filename} does not cover the likelihood frequency range '
            f'[{fmin}, {fmax}] Hz'
        )

    interpolation_frequencies = np.clip(
        frequencies, bayesline_frequencies[0], bayesline_frequencies[-1]
    )
    return np.interp(
        interpolation_frequencies, bayesline_frequencies, bayesline_psd
    )


def plot_psd_comparison(ifo_name, welch_psd, bayesline_psd, frequencies):
    """Write a pre-sampling PSD diagnostic on the likelihood frequency grid."""
    band = (frequencies >= fmin) & (frequencies <= fmax)
    plt.figure(figsize=(10, 6))
    plt.loglog(frequencies[band], welch_psd[band], color='0.5', alpha=0.65,
               linewidth=1.0, label='Welch (32 s Tukey, median)')
    plt.loglog(frequencies[band], bayesline_psd[band], color='tab:blue',
               linewidth=1.2, label='BayesLine posterior median')
    plt.xlim(fmin, fmax)
    plt.xlabel('Frequency (Hz)')
    plt.ylabel(r'Power spectral density ($1/\mathrm{Hz}$)')
    plt.title(f'{grb_name} {ifo_name}: BayesLine and Welch PSD')
    plt.grid(True, which='both', linestyle='--', linewidth=0.5, alpha=0.6)
    plt.legend()
    plt.tight_layout()
    plot_path = os.path.join(output_dir, f'{grb_name}_{ifo_name}_bayesline_vs_welch_psd.png')
...
for ifo_name in successful_detectors:
    ...
    welch_psd = psd_interp_fn(strain_freqs)
    bayesline_psd = load_bayesline_psd(ifo.name, strain_freqs)
    plot_path = plot_psd_comparison(
        ifo.name, welch_psd, bayesline_psd, strain_freqs
    )

    if args.psd_source == 'bayesline':
        selected_psd = bayesline_psd
    else:
        selected_psd = welch_psd

    psd_obj = PowerSpectrum(
        values=jnp.array(selected_psd),
    ...
    print(
        ...
        f"data={t_fetch:.1f}s, "
        f"PSD={t_psd:.1f}s, "
        f"source={args.psd_source}, "
        f"comparison={plot_path}, "
...
for run_name, prior_type in runs:
    ...
    run_label = os.path.join(output_dir,
